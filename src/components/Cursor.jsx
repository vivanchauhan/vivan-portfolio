...
export default function Cursor() {
  ...
  useEffect(() => {
    ...
    const grow = () => {
      ...
      if (ringRef.current)   { ringRef.current.style.width = '56px';   ringRef.current.style.height = '56px'; ringRef.current.style.borderColor = 'rgba(255,107,43,0.8)' }
    }
    const shrink = () => {
      if (cursorRef.current) { cursorRef.current.style.width = '12px'; cursorRef.current.style.height = '12px' }
      if (ringRef.current)   { ringRef.current.style.width = '36px';   ringRef.current.style.height = '36px'; ringRef.current.style.borderColor = 'rgba(255,107,43,0.5)' }
    }

    const els = document.querySelectorAll('a, button, .project-item, .skill-tag, .achievement-card')
